...
def _tab_deprecation(answers: dict, plan: list[dict]) -> None:
    ...
    # Dynamic Rebuild Capex Input
    default_rebuild = answers.get("S5_MAINTENANCE_COST", 6.5) * 3.5
    rebuild_capex = st.slider("Rebuild Capex ($M)", min_value=1.0, max_value=200.0, value=float(default_rebuild), step=0.5)
...
def _tab_memo() -> None:
    ...
    <div style="max-width: 800px;">
        ...
        <div style="margin-top: var(--sp-6); font-size: 11px; color: var(--g500); border-top: 1px solid var(--g200); padding-top: var(--sp-2);">
            Generated by Gemini. Review for accuracy.<br>
            <strong>Run ID:</strong> {st.session_state.get('last_run_id', 'Not logged (local dev)')}
        </div>
    </div>
    """)
...
# ── Tab 5: Assumptions Appendix ──────────────────────────────────────────

def _tab_assumptions() -> None:
    st.html('<div class="hz-report-h2">Model Assumptions & Constants</div>')
    st.html("""
    <p class="hz-p">
    All calculations in the AI Investment Engine are derived from explicit, auditable constants defined in <code>config/value_pools.py</code>. There are no hidden scaling factors or hallucinated ROIs.
    </p>
    """)
    ...
    for k, v in CONSTANTS.items():
        rows += f"<tr><td><strong>{k}</strong></td><td class='num'>{v}</td></tr>"

    st.html(f"""
    <table class="hz-table-wrap">
        <thead>
            <tr>
                <th>Assumption / Constant Name</th>
